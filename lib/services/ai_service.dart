...
→ recommended_start: today, recommended_end: today
...
→ recommended_start: 来週月曜, recommended_end: 期限2日前の平日
...
→ recommended_start: today, recommended_end: 期限前日の金曜
...
→ recommended_start: 2週間後の平日, recommended_end: 期限1週間前の平日
...
→ recommended_start: 1週間後, recommended_end: 期限5日前
...
- "注意が必要です" → 何に注意するか不明''';
  ...
  static Future<AiSortResponse> sortTasks(
    ...
    if (AppConstants.anthropicApiKey.isEmpty) {
      debugPrint('[AI] APIキー未設定 → フォールバック');
    ...
    for (var attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        final result = _parseResponse(response.body, tasks);
