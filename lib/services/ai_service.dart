...
priority別の基準:
- priority 1(緊急): today
- priority 2(要注意): 期限の1〜2日前
- priority 3(通常): 期限の3〜5日前
- priority 4(余裕): 1〜2週間先の適切な日
  ...
  static AiSortResponse _parseResponse(
    ...
    try {
      ...
      final taskResults = (parsed['tasks'] as List).map((r) {
        ...
        final dateResult = _validateDate(
            map['recommended_date'] as String?, today, dueDate);
  ...
  static AiSortResponse _fallbackSort(List<Task> tasks) {
    ...
    final results = tasks.where((t) => t.id != null).map((t) {
      ...
      return AiSortResult(
        ...
        recommendedDate: app_date.formatDateForDb(due),
      );
    }).toList();
